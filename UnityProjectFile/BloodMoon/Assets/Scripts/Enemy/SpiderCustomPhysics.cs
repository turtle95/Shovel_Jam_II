...
public class SpiderCustomPhysics : MonoBehaviour {
  ...
  //  public GameObject graphics;
    ...
    public void WorldGravity()
    {
        ...
        if (!Grounded())
        {
            //tempGroundHit = transform.TransformDirection(hit.point);
            //transform.localPosition = new Vector3(transform.localPosition.x, hit.point.y, transform.localPosition.z);
           // transform.Translate(-transform.up * gravity);
            transform.position += transform.up * gravity * Time.deltaTime; //move forward
        }
