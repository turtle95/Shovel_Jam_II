...
Shader "Particles/FXVille Blood 2017"
{
	Properties
	{
		...
		_FallOffset ("Gravity Offset", range(-1,0)) = -0.5 // should be a value between 0 and -1
	...
	SubShader
	{
		...
		Pass 
		{
            ...
            ZWrite Off
//            Cull Off //turn on for double sided
...
//            #pragma multi_compile FRESNEL_ON FRESNEL_OFF
			...
			v2f vert (appdata v)
			{
				...
				o.uv.zw = o.uv.xy * half2(_Columns, _Rows) + v.texcoord0.z * fixed2(3,8) * _Randomize;
				...
				shade = max(shade, (unity_AmbientSky + unity_AmbientGround + unity_AmbientEquator) * 0.15);
			...
			{	
				...
				////// Sample The Mask //////
				fixed4 mask = tex2D(_MainTex, i.uv.xy * _MainTex_ST.xy + warp);
				mask = saturate(lerp(1, mask, _MaskStr));

				////// Make And Edge Mask So Nothing Spills Off The Quad //////
				fixed2 tempUV = frac(i.uv.xy * half2(_Columns, _Rows)) - 0.5;
				tempUV *= tempUV * 4;
				...
				mask *= edgeMask;
				fixed4 col = max(0.001, i.color);
				col.a = saturate(dot(mask, _ChannelMask));



				////// Sample The Noise //////
				fixed4 noise4 = tex2D(_NoiseTex, i.uv.zw * _NoiseTex_ST.xy + _NoiseTex_ST.zw * i.customData.x + warp);
				fixed noise = dot(noise4, _ChannelMask2);
				noise = saturate(lerp(1,noise,_NoiseAlphaStr));

				////// Alpha Clip //////
				col.a *= noise;
				fixed preClipAlpha = col.a;
				fixed clippedAlpha =  saturate((preClipAlpha * i.color.a - _AlphaMin)/(_AlphaSoft));
				col.a = clippedAlpha;

				////// Bring In Base Lighting //////
				float3 baseLighting = max(0.01,(i.vertLight + 0.2 * dot(i.vertLight, fixed3(1,1,1))));
				baseLighting = i.vertLight.xyz;

				#ifdef SPECULAR_REFLECTION_ON
					////// Sample The Normals //////
					fixed3 normalTex = UnpackNormal(tex2D(_Normal, i.uv.zw * _NoiseTex_ST.xy + _NoiseTex_ST.zw * i.customData.x + warp));
...
//					float fresnel = 1 - normalTex.z * _SpecularColor.w;
				...
				////// Find Edge //////
				fixed edge = 1 - saturate(preClipAlpha * clippedAlpha);
				edge *= edge;
				edge = 1 - edge;
